...
func collectNotificationRows(rows pgx.Rows, limit uint64) ([]entity.Notification, error) {
	notifications := make([]entity.Notification, 0, limit)
...
// GetByID -.
func (r *NotificationRepo) GetByID(ctx context.Context, userID, notificationID string) (entity.Notification, error) {
	sql, args, err := r.Builder.
	...
	if err != nil {
		...
		return entity.Notification{}, fmt.Errorf("NotificationRepo - GetByID - r.Pool.QueryRow: %w", err)
	}
